...
class SFTDataset(Dataset):
    ...
    def _read_files_and_tokenize(self):
        ...
        if len(self.prompt_key) == 1:
            # When there's only one key, dataframe[key] returns a Series which has tolist()
            self.prompts = self.dataframe[self.prompt_key[0]]
        else:
            # When there are multiple keys, use the first one
            # This could be modified to merge multiple columns if needed
            self.prompts = self.dataframe[self.prompt_key[0]] 
