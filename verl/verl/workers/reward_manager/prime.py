...
# This function runs in each worker process and dynamically imports the compute_score function
# instead of trying to unpickle it from the parent process
def _worker_compute_score(module_path, function_name, task, completion, reference, task_extra_info):
    # Dynamically import the module in the worker process
    ...
    return func(task, completion, reference, task_extra_info)


async def single_compute_score(evaluation_func, completion, reference, task, task_extra_info, executor, timeout=300.):
    loop = asyncio.get_running_loop()
    try:
        # Get module and function name information if this is a custom function
        if hasattr(evaluation_func, '__module__') and hasattr(evaluation_func, '__name__'):
            module_path = evaluation_func.__module__
            function_name = evaluation_func.__name__
            
            # Use the worker function that will import the module in the subprocess
            tasks = [
                asyncio.wait_for(
                    loop.run_in_executor(
                        executor,
                        partial(_worker_compute_score, module_path, function_name, task, completion, reference, task_extra_info)
                    ),
                    timeout=timeout)
            ]
        else:
            # Fallback to original method if not a standard function
            tasks = [
                asyncio.wait_for(
                    loop.run_in_executor(
                        executor,
